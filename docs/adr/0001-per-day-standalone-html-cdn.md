...
7. **預設不用框架（vanilla JS）**：GSAP / Three.js 皆為 vanilla，預設不引入 Vue/Vite。**例外**：Day 21（`gsap.context()` + `contextSafe()` + `matchMedia`）主題本身就是「動畫在框架元件生命週期中正確清理」，該天（可能加 Day 25 Three.js 整合）用小型 Vue 環境示範，這是履歷差異化亮點。
