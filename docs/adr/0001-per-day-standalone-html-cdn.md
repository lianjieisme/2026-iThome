...
6. **樣式預設原生 CSS**：demo 的靜態外觀預設用原生 CSS。版面較複雜的天（如 Day 14 捲動敘事、Day 28 跑馬燈）可選用 **Tailwind CDN**（一行引入、免 build）。不建 Tailwind build。
7. **預設不用框架（vanilla JS）**：GSAP / Three.js 皆為 vanilla，預設不引入 Vue/Vite。**例外**：Day 18（`gsap.context()` + `matchMedia`）主題本身就是「動畫在框架元件生命週期中正確清理」，該天（可能加 Day 24 Three.js 整合）用小型 Vue 環境示範——這是履歷差異化亮點。
...
- ⚠️ Three.js（Day 21-26）需用 CDN + import map，稍微多一點設定，但仍不需要 build 工具。
